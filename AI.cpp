...
void AI::move_scouters_to_find_castle(map<int, char>& order, vector<Unit>& remain_workers, const vector<Unit>& enemy_units, const Pos& my_castle_pos)
{
...
void AI::defend_castle(map<int, char>& order, const InputResult& input, vector<Unit>& remain_workers, int& remain_resources)
{
...
void AI::select_workers_for_resource(const vector<Unit>& my_workers)
{
...
void AI::move_workers_for_resources(map<int, char>& order, const InputResult& input, vector<Unit>& remain_workers, int& remain_resources)
{
    const Unit my_castle = input.get_my({CASTLE})[0];
    const auto& my_villages = input.get_my({VILLAGE});
...
void AI::move_scouters(map<int, char>& order, const InputResult& input, vector<Unit>& remain_workers, int& remain_resources)
{
    ...
    if (enemy_castle.id == -1)
    {
        ...
        move_scouters_to_find_castle(order, remain_workers, enemy_units, my_castle.pos);
...
map<int, char> AI::solve(const InputResult& input)
{
    ...
    if (input.current_turn < 200)
    {
        ...
        if (found)
            defend_castle(order, input, remain_workers, remain_resources);
    }

    select_workers_for_resource(my_workers);

    move_workers_for_resources(order, input, remain_workers, remain_resources);
...
//     move_scouters(order, input, remain_workers, remain_resources);

    // move_workers_for_mapping
    {
